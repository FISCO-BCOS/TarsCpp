...
public:
  ...
  static void readXml(std::map<std::string, V, Cmp, Alloc> &m,
                      const XmlValuePtr &p, bool isRequire = true) {
    if (NULL != p.get() && p->getType() == eXmlTypeObj) {
      XmlValueObjPtr pObj = XmlValueObjPtr::dynamicCast(p);
      std::map<std::string, XmlValuePtr>::iterator iter;
...
public:
  ...
  static XmlValueStringPtr writeXml(const char *buf, const UInt32 len,
                                    bool cdata = false) {
    return (new XmlValueString(std::string(buf, len), cdata));
  }

  template <typename V, typename Cmp, typename Alloc>
  static XmlValueObjPtr
  writeXml(const std::map<std::string, V, Cmp, Alloc> &m) {
