...
public:
  ...
  void onConnect(const TC_Endpoint &ep) {
    _onconnect = true;
    //		LOG_CONSOLE_DEBUG << "onConnect:" << ep.toString() << ", " <<
    //_onconnect << ", " << this << endl;
  }
...
struct ClientHelloCallback : public HelloPrxCallback {
  ClientHelloCallback(int64_t t, int i, int c, const std::string &buff,
                      std::atomic<int> &count)
      : start(t), cur(i), count(c), buffer(buff), callback_count(count) {}

  ClientHelloCallback(HelloPrx prx, std::atomic<int> &count)
      : _prx(prx), callback_count(count) {}

  //回调函数
  virtual void callback_testTrans(int ret, const std::string &r);

  //回调函数
  virtual void callback_testHello(int ret, const std::string &r);
  ...
  virtual void callback_testTimeout(int ret) { callback = true; }
...
struct HelloHttpCallback : public HttpCallback {
  HelloHttpCallback(int64_t t, int i, int c, const std::string &buff,
                    std::atomic<int> &count)
      : start(t), cur(i), count(c), _buff(buff), callback_count(count) {}
  ...
  virtual int onHttpResponse(const shared_ptr<TC_HttpResponse> &rsp) {
    ...
    if (cur == count - 1) {
      ...
      LOG_CONSOLE_DEBUG << "count:" << count << ", " << cost
...
public:
  ...
  template <typename T>
  void startServer(T &server, TC_Config conf,
                   TC_EpollServer::SERVER_OPEN_COROUTINE openCoroutine =
                       TC_EpollServer::NET_THREAD_MERGE_HANDLES_THREAD) {
    conf.set("/tars/application/server<opencoroutine>",
             TC_Common::tostr(openCoroutine));

    ASSERT_TRUE(conf.get("/tars/application/server<opencoroutine>") ==
                TC_Common::tostr(openCoroutine));
  ...
  void startServer(HelloServer &server,
                   TC_EpollServer::SERVER_OPEN_COROUTINE openCoroutine);

  template <typename T> void stopServer(T &server) {
    server.terminate();
    server.getThreadControl().join();
  }

  TC_Endpoint getEndpoint(const std::string &adapter);

  string getObj(TC_Config &conf, const std::string &adapter) {
    return conf.get("/tars/application/server/" + adapter + "<servant>") + "@" +
           conf.get("/tars/application/server/" + adapter + "<endpoint>");
  }

  TC_Endpoint getEndpoint(TC_Config &conf, const std::string &adapter) {
    return TC_Endpoint(
        conf.get("/tars/application/server/" + adapter + "<endpoint>"));
  }
  ...
  template <typename T>
  T getObj(Communicator *comm, const std::string &adapter) {
  ...
  int getFdCounts() {
    ...
    vector<std::string> fds;
    TC_File::listDirectory("/proc/" + TC_Common::tostr(getpid()) + "/fd", fds,
                           false);
    return fds.size();
  }
  ...
  void syncCustom(Communicator *comm,
                  const std::string &adapter = "CustomAdapter");

  void asyncCustom(Communicator *comm,
                   const std::string &adapter = "CustomAdapter");

  void testPush(Communicator *comm, const std::string &adapter = "PushAdapter");

  void testReconnect(Communicator *comm,
                     const std::string &adapter = "PushAdapter");

  void checkWup();

  void checkSyncHttp(Communicator *comm,
                     const std::string &adapter = "HttpAdapter",
                     bool close = false);

  void checkASyncHttp(Communicator *comm,
                      const std::string &adapter = "HelloAdapter",
                      bool close = false);

  void checkSync(Communicator *comm,
                 const std::string &adapter = "HelloAdapter");

  void checkSyncOnce(HelloPrx prx);

  void checkASyncOnce(HelloPrx prx); // Communicator *comm, const
                                     // std::string&adapter = "HelloAdapter");

  void checkASync(Communicator *comm,
                  const std::string &adapter = "HelloAdapter");
  ...
  void transGlobalCommunicator(function<void(Communicator *)> func,
                               Communicator *comm);
  ...
  void transComplexCommunicator(function<void(Communicator *)> func,
                                Communicator *comm);
  ...
  void transInCoroutineGlobalCommunicator(function<void(Communicator *)> func,
                                          Communicator *comm);
  ...
  void transInCoroutineComplexCommunicator(function<void(Communicator *)> func,
                                           Communicator *comm);
