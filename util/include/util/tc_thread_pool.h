...
struct TC_ThreadPool_Exception : public TC_Exception {
  ...
  TC_ThreadPool_Exception(const std::string &buffer, bool err)
...
public:
   ...
   */
  template <class F, class... Args>
  auto exec(const std::string &hashkey, F &&f, Args &&...args)
      -> std::future<decltype(f(args...))> {
   ...
   */
  template <class F, class... Args>
  auto exec(const std::string &hashkey, int64_t timeoutMs, F &&f,
