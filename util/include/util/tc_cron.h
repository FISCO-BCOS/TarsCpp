...
public:
  ...
  //	TC_CronException(const std::string& buffer, bool err) : TC_Exception(buffer,
  //err) {};
...
protected:
   ...
   */
  static std::string
  replaceOrdinals(std::string text,
  ...
  makeRange(std::string field, cron_int minval, cron_int maxval);
  ...
  template <size_t N>
  static size_t
  findNext(const std::bitset<N> &target, std::tm &tmt, unsigned int minimum,
           unsigned int maximum, unsigned int value, CronField field,
           CronField next_field, const std::bitset<7> &marked_fields);
