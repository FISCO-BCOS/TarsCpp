...
static TC_NetWorkBuffer::PACKET_TYPE parseLine(TC_NetWorkBuffer &in,
                                               std::vector<char> &out) {
  try {
    if (in.empty()) {
...
public:
   ...
   * 初始化
   */
  virtual void initialize() { _server = (MyTcpServer *)(getEpollServer()); }
  ...
  virtual void handle(const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      ...
      //			 LOG_CONSOLE_DEBUG <<
      // TC_Thread::CURRENT_THREADID() << ", handle:" <<
      // string(data->buffer().data(), data->buffer().size()) << endl;
  ...
  virtual void
  handleClose(const shared_ptr<TC_EpollServer::RecvContext> &data) {
...
public:
  ...
  virtual void handle(const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      ...
      std::stringbuff(data->buffer().data(), data->buffer().size());
...
class MyTcpServer {
public:
  MyTcpServer() { _epollServer = new TC_EpollServer(); };

  void initialize() {}
#if TARS_SSL
  void bindSSL(const std::string &str, int maxConnections = 10240) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpHandle>("SSLAdapter", str, 5);
    ...
    shared_ptr<TC_OpenSSL::CTX> ctx =
        TC_OpenSSL::newCtx(CA, SERVER_CERT, SERVER_KEY, verifyClient, ciphers);
  ...
  void bindUdp(const std::string &str, int maxConnections = 10240) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpHandle>("UdpAdapter", str, 2);
  ...
  void bindManualTcp(const std::string &str, int maxConnections = 10240) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpHandle>("TcpManualAdapter", str, 5);
  ...
  void bindTcp(const std::string &str, int maxConnections = 10240) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpHandle>("TcpAdapter", str, 5);
  ...
  void bindTcpLine(const std::string &str, int maxConnections = 10240) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpHandle>("TcpLineAdapter", str, 5);
  ...
  void bindTcpQueue(const std::string &str) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<TcpQueueHandle>("TcpQueueAdapter", str,
                                                        5);
  ...
  void waitForShutdown() {
    _start = new std::thread([=] { _epollServer->waitForShutdown(); });
...
public:
   ...
   * 初始化
   */
  virtual void initialize() { _server = (MyHttpServer *)(getEpollServer()); }
  ...
  virtual void handle(const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      ...
      // cout << "-------------------------------------------------------" <<
      // endl; cout << "request: " << string(data->buffer().data(),
      // data->buffer().size()) << endl; cout <<
      // "-------------------------------------------------------" << endl;
      ...
      std::stringbuffer = response.encode();
...
class MyHttpServer {
public:
  MyHttpServer() { _epollServer = new TC_EpollServer(); };

  void initialize() {}

  void bindHttp(const std::string &str) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<HttpHandle>(
            "HttpAdapter", str,
            5); // new TC_EpollServer::BindAdapter(_epollServer);
  ...
  void bindHttpSSL(const std::string &str) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<HttpHandle>("HttpsAdapter", str, 5);
  ...
  void bindHttpSSLVerify(const std::string &str) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        _epollServer->createBindAdapter<HttpHandle>("HttpsVerifyAdapter", str,
                                                    5);
    ...
    shared_ptr<TC_OpenSSL::CTX> ctx = TC_OpenSSL::newCtx(
        CLIENT_CERT, SERVER_CERT, SERVER_KEY, verifyClient, ciphers);
